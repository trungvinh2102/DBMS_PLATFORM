...
class AIService:
            ...
            try:
                ...
                if not sql:
                    ...
                    if agent_response.get("summary") or agent_response.get("thinking"):
                        ...
                        # Save meta-task response to conversation
                        self._save_chat("user", prompt, user_id, db_id, conv_id=conv_id)
                        aid = self._save_chat("assistant", agent_response.get("summary") or agent_response.get("thinking") or "Analysis complete.", user_id, db_id, conv_id=conv_id)
                        agent_response["messageId"] = aid
                        return agent_response
    ...
    def _save_chat(self, role: str, content: str, user_id: Optional[str] = None, db_id: Optional[str] = None, conv_id: Optional[str] = None) -> Optional[str]:
        """Persists AI chat messages to the database and returns the generated ID."""
        session = SessionLocal()
        try:
