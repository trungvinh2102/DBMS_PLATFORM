...
export function useConnectionsPage() {
  ...
  const [activeConn, setActiveConn] = useState<DataSource | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: (data: Partial<DataSource>) => databaseApi.create(data),
  });
    ...
    try {
      ...
      if (editingId) {
        ...
        await updateMutation.mutateAsync({
          id: editingId,
          databaseName: formData.name || formData.database,
          type: selectedType,
          config: updateConfig,
        });
  ...
  const handleOpenUpdate = (conn: DataSource) => {
